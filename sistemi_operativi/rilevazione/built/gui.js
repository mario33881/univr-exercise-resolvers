 ...
 */
observeDOM(document.querySelector("#input_table_body"), function (m) {
...
function appendAfter(element, reference) {
...
function add_table_col() {
    ...
    g_n_table_columns += 2;
    const header = rows[0];
    const header_elements = Array.from(header.children);
...
function remove_table_col() {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        (_a = rows[i].querySelector(".separator")) === null || _a === void 0 ? void 0 : _a.remove();
    }
    g_n_table_columns -= 2;
}
function parse_table_input() {
    ...
    for (const row of tbody.children) {
        ...
        for (const column of row.children) {
            ...
            if ((input === null || input === void 0 ? void 0 : input.valueAsNumber) !== undefined)
                values[values.length - 1].push(input === null || input === void 0 ? void 0 : input.valueAsNumber);
            console.log(input === null || input === void 0 ? void 0 : input.valueAsNumber);
        }
        console.log("------");
    }
    return values;
...
function create_resource_arrays(matrix) {
    const result = { "alloc": [], "request": [] };
    let n_cols_per_array = Math.floor(matrix[0].length / 2);
    for (const row of matrix) {
        const alloc_resources = [];
